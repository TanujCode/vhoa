# fix_user.py — backend folder mein rakho, ek baar chalao, phir delete karo

user_router = '''from fastapi import APIRouter, Depends, HTTPException, UploadFile, File
...
def delete_picture(
    ...
    if not current_user.user_profile_url:
        raise HTTPException(400, "Koi picture nahi hai.")
    old = current_user.user_profile_url.lstrip("/")
    if os.path.exists(old): os.remove(old)
...
with open('app/routers/user.py', 'w', encoding='utf-8') as f:
    f.write(user_router)
print('app/routers/user.py written successfully!')

main_content = open('app/main.py', encoding='utf-8').read()
if '# app.include_router(user.router' in main_content:
    ...
    print('main.py router uncommented!')
else:
    print('main.py already has user router or check manually.')