...
const Payments = ({ community, user, paymentState, setPaymentState, viewAsResident }) => {
  ...
  return (
    <div className="pb-12 text-slate-900 dark:text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5 pb-3 border-b border-slate-200/60 dark:border-white/5">
        <h1 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
          Payments & Escrow
        </h1>
        {community?.bank_name ? (
          <div className="bg-blue-500/10 border border-blue-500/20 px-3 py-1.5 rounded-xl flex items-center gap-2 flex-wrap">
            <span className="text-blue-600 dark:text-blue-400 font-semibold text-[10px] py-0.5 px-2 bg-blue-500/10 dark:bg-blue-500/20 rounded-full">ESCROW ACTIVE</span>
            <div className="text-xs">
              <span className="font-semibold text-slate-700 dark:text-gray-300">{community.bank_name}</span>
              <span className="text-slate-400 dark:text-gray-500 font-mono ml-2">(*{community.bank_account_no?.slice(-4) || 'XXXX'})</span>
            </div>
          </div>
        ) : (
          <div className="bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-xl flex items-center gap-2 flex-wrap">
            <span className="text-amber-600 dark:text-amber-400 font-semibold text-[10px] py-0.5 px-2 bg-amber-500/10 dark:bg-amber-500/20 rounded-full">NO BANK SETUP</span>
            <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">
              Escrow bank account setup required in Settings.
            </span>
          </div>
        )}
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-2 p-1.5 bg-gradient-to-br from-slate-50 to-blue-50 dark:from-[#1E2E42] dark:to-[#162535] border border-slate-200/80 dark:border-white/10 rounded-2xl mb-6 w-fit overflow-x-auto max-w-full shadow-sm">
        <button
          onClick={() => setActiveTab('pay')}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all whitespace-nowrap ${
            activeTab === 'pay' ? 'bg-[#1D68DF] hover:bg-[#15459E] text-white hover:text-white shadow-lg shadow-[#1D68DF]/25' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all whitespace-nowrap ${
            activeTab === 'recurring' ? 'bg-[#1D68DF] hover:bg-[#15459E] text-white hover:text-white shadow-lg shadow-[#1D68DF]/25' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all whitespace-nowrap ${
            activeTab === 'history' ? 'bg-[#1D68DF] hover:bg-[#15459E] text-white hover:text-white shadow-lg shadow-[#1D68DF]/25' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5'
          }`}
        >
        ...
        {isBoardOrAdmin && (
          <>
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all whitespace-nowrap ${
                activeTab === 'virtualhoa' ? 'bg-[#1D68DF] hover:bg-[#15459E] text-white hover:text-white shadow-lg shadow-[#1D68DF]/25' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all whitespace-nowrap ${
                activeTab === 'vendors' ? 'bg-[#1D68DF] hover:bg-[#15459E] text-white hover:text-white shadow-lg shadow-[#1D68DF]/25' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5'
              }`}
            >
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <Loader className="animate-spin text-[#5BA4F5]" size={40} />
        ...
        <div>
          {dues.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {dues.map((due, idx) => (
                <div key={idx} className="bg-gradient-to-br from-slate-50 to-blue-50 dark:from-[#1E2E42] dark:to-[#162535] border border-slate-200/80 dark:border-white/10 rounded-3xl p-6 flex flex-col justify-between hover:border-blue-500/30 transition shadow-sm hover:shadow-md relative overflow-hidden group">
                  ...
                  <div>
                    <span className="text-[10px] font-mono tracking-wider bg-slate-100 dark:bg-[#1E3248] text-blue-600 dark:text-blue-400 px-3 py-1 rounded-full font-bold uppercase">
                      {due.reason.replace('_', ' ')}
                    </span>
                    <h3 className="text-lg font-semibold mt-4 text-slate-800 dark:text-white line-clamp-2">{due.title}</h3>
                    <div className="mt-2 text-2xl font-bold font-mono text-blue-600 dark:text-blue-400">${due.amount.toFixed(2)}</div>
                    {due.due_date && (
                  ...
                  <button
                    ...
                    className="w-full mt-6 flex items-center justify-center gap-2 bg-[#1D68DF] hover:bg-[#5BA4F5] text-white py-3 rounded-2xl font-medium transition shadow-lg shadow-[#1D68DF]/20 group-hover:scale-[1.02]"
                  >
      ...
      {!loading && activeTab === 'recurring' && (
        <div className="max-w-2xl bg-gradient-to-br from-slate-50 to-blue-50 dark:from-[#1E2E42] dark:to-[#162535] border border-slate-200/80 dark:border-white/10 rounded-3xl p-8 shadow-sm">
          ...
          {recurring ? (
            <div className="bg-slate-50 dark:bg-[#1E3248] border border-slate-200 dark:border-blue-500/20 rounded-2xl p-6">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest bg-blue-500/10 dark:bg-blue-500/20 px-3 py-1 rounded-full">Auto-Pay Active</span>
            ...
            <form onSubmit={handleSetupRecurring} className="space-y-6">
              ...
              <div id="recurring-bank-fields" style={{ display: 'none' }} className="space-y-4 bg-slate-50 dark:bg-[#1E3248] rounded-2xl p-4 border border-slate-200 dark:border-white/5">
                <h4 className="text-sm font-medium text-blue-600 dark:text-blue-400">ACH Bank Transfer Details</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-100 dark:bg-[#0D1B2A] border border-slate-200 dark:border-white/20 rounded-xl p-2.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-[#1D68DF]"
                    />
                  </div>
                  <div>
              ...
              <button
                type="submit"
                className="w-full bg-[#1D68DF] hover:bg-[#5BA4F5] text-white py-4 rounded-2xl font-semibold transition shadow-lg shadow-[#1D68DF]/25"
              >
                Enable Auto-Pay
              </button>
            </form>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-sm text-slate-700 dark:text-gray-300">
                  {history.map((h) => (
                    <tr key={h.payment_id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-3 py-4 text-right font-mono font-semibold text-blue-600 dark:text-blue-400 whitespace-nowrap">
                        ${h.amount.toFixed(2)}
                      </td>
                      <td className="px-3 py-4 text-center whitespace-nowrap">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${
                          h.status === 'COMPLETED' ? 'bg-blue-500/10 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400' : 'bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400'
                        }`}>
                          {h.status}
                        </span>
                      </td>
          ...
          <div className="bg-gradient-to-br from-slate-50 to-blue-50 dark:from-[#1E2E42] dark:to-[#162535] border border-slate-200/80 dark:border-white/10 rounded-3xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {nestBloqDues.map((item) => (
                <div key={item.id} className="bg-slate-50 dark:bg-[#1E3248] rounded-2xl p-6 flex flex-col justify-between border border-slate-200 dark:border-white/5 hover:border-blue-500/20 transition shadow-sm hover:shadow-md relative overflow-hidden group">
                  <div>
                    <span className="text-[10px] font-mono tracking-wider bg-slate-200 dark:bg-[#0D1B2A] text-blue-700 dark:text-blue-400 px-3 py-1 rounded-full font-bold uppercase">
                      SYSTEM LICENSE
                    </span>
                    <h3 className="text-lg font-semibold mt-4 text-slate-800 dark:text-white">{item.title}</h3>
                    <div className="mt-2 text-2xl font-bold font-mono text-blue-600 dark:text-blue-400">${item.amount.toFixed(2)}</div>
                    <div className="text-xs text-slate-500 dark:text-gray-400 mt-4 flex items-center gap-1.5">
                      <Calendar size={13} />
                  ...
                  {item.status === 'COMPLETED' ? (
                    ...
                    <button
                      ...
                      className="w-full mt-6 flex items-center justify-center gap-2 bg-[#1D68DF] hover:bg-[#5BA4F5] text-white py-3 rounded-2xl font-medium transition"
                    >
            ...
            {vendorAssignments.filter(v => v.status === 'APPROVED' || v.status === 'COMPLETED').length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {vendorAssignments.filter(v => v.status === 'APPROVED' || v.status === 'COMPLETED').map((v) => (
                  <div key={v.assignment_id} className="bg-slate-50 dark:bg-[#1E3248] rounded-2xl p-6 flex flex-col justify-between border border-slate-200 dark:border-white/5 hover:border-blue-500/20 transition shadow-sm hover:shadow-md relative overflow-hidden group">
                    <div>
                      <div className="flex justify-between items-start">
                        <span className={`text-[10px] font-mono tracking-wider px-3 py-1 rounded-full font-bold uppercase ${
                          v.status === 'COMPLETED' ? 'bg-blue-500/10 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400' : 'bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-300'
                        }`}>
                      ...
                      <div className="mt-2 text-2xl font-bold font-mono text-blue-600 dark:text-blue-400">${(v.quote_amount || 0).toFixed(2)}</div>
            ...
            <div className="p-8">
              ...
              {wizard.currentStep === 1 && (
                <div>
                  ...
                  <div className="bg-slate-50 dark:bg-[#1E3248] border border-slate-200 dark:border-0 rounded-2xl p-6 space-y-4 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-200 dark:border-white/5">
                      <div>
                        ...
                        <div className="text-2xl font-bold font-mono text-blue-600 dark:text-blue-400 mt-1">${wizard.dueItem.amount.toFixed(2)}</div>
                      </div>
                      <div>
                  ...
                  <p className="text-xs mb-6 flex items-center gap-2 bg-[#1D68DF]/10 border border-[#1D68DF]/20 p-3 rounded-xl text-[#1D68DF] dark:text-[#5BA4F5]">
                  ...
                  <button
                    ...
                    className="w-full bg-[#1D68DF] hover:bg-[#5BA4F5] text-white py-4 rounded-2xl font-semibold transition"
                  >
                ...
                <div>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all text-left ${
                        wizard.paymentMethod === 'PAYPAL' ? 'border-[#1D68DF] bg-[#1D68DF]/5 text-[#1D68DF] dark:text-white' : 'border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-700 dark:text-gray-300'
                      }`}
                    >
                      ...
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                        wizard.paymentMethod === 'PAYPAL' ? 'border-[#1D68DF] bg-[#1D68DF]' : 'border-slate-300 dark:border-gray-500'
                      }`}>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all text-left ${
                        wizard.paymentMethod === 'VISA_CHECKOUT' ? 'border-[#1D68DF] bg-[#1D68DF]/5 text-[#1D68DF] dark:text-white' : 'border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-700 dark:text-gray-300'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                      ...
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                        wizard.paymentMethod === 'VISA_CHECKOUT' ? 'border-[#1D68DF] bg-[#1D68DF]' : 'border-slate-300 dark:border-gray-500'
                      }`}>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all text-left ${
                        wizard.paymentMethod === 'BANK_TRANSFER' ? 'border-[#1D68DF] bg-[#1D68DF]/5 text-[#1D68DF] dark:text-white' : 'border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-700 dark:text-gray-300'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                      ...
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                        wizard.paymentMethod === 'BANK_TRANSFER' ? 'border-[#1D68DF] bg-[#1D68DF]' : 'border-slate-300 dark:border-gray-500'
                      }`}>
                  ...
                  {wizard.paymentMethod === 'BANK_TRANSFER' && (
                    <div className="space-y-3 mb-6 bg-slate-50 dark:bg-[#1E3248] rounded-2xl p-4 border border-slate-200 dark:border-white/5">
                      <h4 className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide">ACH Wire Account Info</h4>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-100 dark:bg-[#0D1B2A] border border-slate-200 dark:border-white/20 rounded-xl px-3 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                          />
                        </div>
                        <div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      className="w-2/3 bg-[#1D68DF] hover:bg-[#5BA4F5] text-white py-4 rounded-2xl font-semibold transition disabled:opacity-50"
                    >
                      Secure Pay
                    </button>
                  </div>
              ...
              {wizard.currentStep === 3 && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <Loader className="animate-spin text-[#5BA4F5] mb-6" size={50} />
                ...
                <div className="text-center">
                  ...
                  <div className="bg-slate-50 dark:bg-[#1E3248] border border-slate-200 dark:border-0 rounded-2xl p-6 my-6 text-left space-y-3 font-mono text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-blue-600 dark:text-blue-400 text-right select-all">{wizard.receipt.gateway_token}</span>
                    </div>
