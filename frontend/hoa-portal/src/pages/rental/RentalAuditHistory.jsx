...
import {
  RefreshCw, Search, Shield, User, Globe, Layers,
  Lock, Building2, AlertTriangle, Wrench, Calendar,
  Briefcase, Megaphone, CreditCard, FileText, ChevronDown, Clock, DollarSign
} from 'lucide-react';
...
const MODULE_META = {
  ...
  payment:         { icon: DollarSign,    bg: 'bg-emerald-100 dark:bg-emerald-900/30', text: 'text-emerald-600 dark:text-emerald-400' },
};
...
const MODULES = ['', 'auth', 'user', 'payment', 'service_request', 'vendor', 'community'];
const labelOf = (m) => m ? m.split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ') : 'All Actions';
...
const RentalAuditHistory = ({ user }) => {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      let res;
      if (viewMode === 'my') {
        res = await API.get('/rental/audit/my?limit=100');
      } else {
    ...
    <div className="text-slate-900 dark:text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Track all activity across your rental portfolio</p>
        </div>
          ...
          {isAdmin && (
            <div className="flex rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 text-xs font-semibold">
              {[
                ...
                { id: 'my',  label: 'My Activity', icon: User   },
              ].map(v => (
                <button
                  key={v.id}
      ...
      {/* ── List ─────────────────────────────────────────────── */}
      <AuditList filtered={filtered} logs={logs} loading={loading} isAdmin={isAdmin} />
    </div>
  );
};

/* ── Activity List ───────────────────────────────────────── */
const AuditList = ({ filtered, logs, loading, isAdmin }) => {
