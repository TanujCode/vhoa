...
const DocumentModal = ({ communityId, onClose, onSuccess, showAlert }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.detail || 'Error uploading document. Only PDF, DOC, DOCX, Images, and Excel files are allowed (Max 10MB).');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-[#1E2E42] rounded-3xl border border-slate-200 dark:border-white/10 w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-slate-200 dark:border-white/10 flex justify-between items-center bg-slate-50 dark:bg-[#162535]">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
            <Plus size={18} className="text-blue-500" /> Upload Condo Document
          </h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-650 dark:hover:text-white font-semibold text-sm"></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              required
              placeholder="e.g. Building Safety Audit 2026"
              value={docName}
...
export default function CondoDocuments({ community, user }) {
  ...
  const role = (user?.role_name || user?.role || 'resident').toLowerCase();
  ...
  const fetchDocuments = async () => {
    try {
      ...
      console.error('Condo Documents fetch error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="text-slate-900 dark:text-white font-sans max-w-7xl mx-auto p-2">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          {isManagement && (
            <button
              ...
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold transition flex items-center gap-1.5 shadow-md shadow-blue-500/25 whitespace-nowrap cursor-pointer"
            >
      ...
      <div className="bg-gradient-to-br from-slate-50 to-blue-50 dark:from-[#1E2E42] dark:to-[#162535] border border-slate-200/80 dark:border-white/10 rounded-3xl overflow-hidden shadow-sm animate-fadeIn">
        ...
        {loading && documents.length === 0 ? (
          ...
          <div className="p-20 text-center text-slate-500 dark:text-gray-400 bg-white/40 dark:bg-transparent">
            ...
            <p className="text-xs text-slate-450 mt-1">There are no uploaded documents in this category.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-white/10 text-slate-500 dark:text-gray-400 text-xs uppercase tracking-wider">
                  ...
                  <th className="text-left px-5 py-4 whitespace-nowrap">Category</th>
                  <th className="text-left px-5 py-4 whitespace-nowrap">Uploaded Date</th>
                  <th className="text-right px-5 py-4 whitespace-nowrap">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredDocs.map(doc => (
                  <tr key={doc.document_id} className="border-b border-slate-100 dark:border-white/5 hover:bg-slate-100/50 dark:hover:bg-white/5 transition-all">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <button
                            ...
                            className="font-medium text-slate-800 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 hover:underline transition-all block truncate max-w-xs sm:max-w-md text-sm text-left font-sans"
                          >
                            {doc.document_name}
                          </button>
                          <p className="text-xs text-slate-450 mt-0.5 truncate max-w-[200px] sm:max-w-xs">{doc.document_url.split('/').pop()}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-slate-500 dark:text-gray-400 text-xs whitespace-nowrap">
                    ...
                    <td className="px-5 py-4 whitespace-nowrap">
                      <div className="flex justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => handleView(getBaseUrl(doc.document_url), doc.document_name)}
                          className="p-2 bg-blue-500/10 hover:bg-blue-500/20 dark:bg-blue-500/20 dark:hover:bg-blue-500/30 text-blue-600 dark:text-blue-400 rounded-xl transition-all cursor-pointer"
                          title="View Document"
                        >
                          <Eye size={16} />
                        </button>
                        <button
                          ...
                          className="p-2 bg-slate-100 hover:bg-slate-205 dark:bg-white/10 dark:hover:bg-white/20 text-slate-600 dark:text-gray-300 rounded-xl transition-all cursor-pointer"
                        ...
                        {isManagement && (
                          <button
                            ...
                            className="p-2 bg-red-500/10 hover:bg-red-500/20 dark:bg-[#3B1C1C] dark:hover:bg-[#5C2323] text-red-600 dark:text-red-400 rounded-xl transition-all cursor-pointer"
                            title="Delete"
                          >
                            <Trash2 size={16} />
                          </button>
