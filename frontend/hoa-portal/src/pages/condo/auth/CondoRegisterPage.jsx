...
export default function CondoRegisterPage() {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await API.post('/condo/auth/register', {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        setTimeout(() => {
          navigate('/condo/verify-otp', { state: { email: data.email } });
        }, 2000);
      }
    } catch (err) {
  ...
  return (
    <AuthLayout>
      ...
      {successMsg && (
        <div className="mb-2 p-3 bg-green-50 border border-green-200 text-green-600 text-sm rounded-lg">
          {successMsg}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-2.5">
        ...
        >
          {isSubmitting ? 'Registering...' : 'Register Condo Account'}
        </button>
      </form>
