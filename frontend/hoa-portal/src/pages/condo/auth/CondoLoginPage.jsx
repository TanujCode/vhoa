...
export default function CondoLoginPage() {
  ...
  const trigger2FAVerification = async (code) => {
    ...
    setSuccessMsg('');
    try {
      const res = await API.post('/condo/auth/login/verify-2fa', {
        email_id: email2FA,
        otp_code: code,
      });
      ...
      if (res.data && res.data.access_token) {
        ...
        localStorage.setItem('condo_token', res.data.access_token);
        if (res.data.session_token) {
          localStorage.setItem('condo_session_token', res.data.session_token);
        }
        localStorage.setItem('condo_user', JSON.stringify(userData));
  ...
  const handleResend2FA = async () => {
    ...
    setSuccessMsg('');
    try {
      await API.post('/condo/auth/otp/send', {
        email_id: email2FA
      });
